...
def truncate_table(schema: str, table_name: str, engine) -> None:
...
def table_exists(schema: str, table_name: str, engine) -> bool:
...
def load_table(csv_path: Path, schema: str, engine) -> int:
    ...
    if not table_exists(schema, table_name, engine):
    ...
    try:
        ...
        truncate_table(schema, table_name, engine)
...
def main() -> None:
    ...
    try:
        ...
        for csv_file in csv_files:
            try:
                rows = load_table(csv_file, schema=RAW_SCHEMA, engine= engine)
        ...
        if failure_count > 0:
            raise RuntimeError(f"Raw load failed for {failure_count} file(s)")
